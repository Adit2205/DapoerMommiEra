...
if (empty($_SESSION['username'])){
	...
	include "../conn.php";
    $_SESSION['user_id'];
    $cek = mysqli_query($koneksi, "SELECT * FROM customer ");
?>
<!DOCTYPE html>
<html>
    <head>
...
$query1 = "SELECT po_terima.id, po_terima.nopo, produk.nama, po_terima.tanggal, po_terima.qty, po_terima.total 
           FROM po_terima 
           INNER JOIN produk ON po_terima.kode = produk.kode 
           WHERE po_terima.kd_cus='$kodeku'";
...
$kd = $_SESSION['user_id'];
$koneksi = mysqli_connect("localhost", "root", "", "warnasol");

// Function to update status if not confirmed within 1 hour
function updateStatusIfTimeout($koneksi, $order) {
    $current_time = new DateTime();
    $order_time = new DateTime($order['tanggal']);
    $interval = $current_time->diff($order_time);

    // If the difference is greater than or equal to 1 hour and status is still 'Belum'
    if ($interval->h >= 1 && $order['status'] == 'Belum') {
        $update_query = "UPDATE konfirmasi SET status='Dibatalkan' WHERE id_kon=" . $order['id_kon'];
...
$query3 = "SELECT konfirmasi.*, produk.nama AS nama_produk 
           FROM konfirmasi 
           INNER JOIN produk ON konfirmasi.kode = produk.kode 
           WHERE kd_cus='$kd'";
$hasil2 = mysqli_query($koneksi, $query3) or die(mysqli_error($koneksi));
?>
<table id="example" class="table table-hover table-bordered">
    <thead>
        <tr>
    ...
    <?php while($data2 = mysqli_fetch_array($hasil2)) { 
        $data2 = updateStatusIfTimeout($koneksi, $data2);
    ?>
        <tr>
            ...
            <td><center><?php
                ...
                else if ($data2['status'] == 'Dibatalkan'){
                    echo '<span class="label label-danger">Dibatalkan</span>';
                }
                else if ($data2['status'] == 'Tidak Valid'){
                    echo '<span class="label label-danger">Tidak Valid</span>';
                }
            ?>
            </center></td>
            <td><center>
                <div id="thanks">
    <?php if ($data2['status'] != 'Dibatalkan' && $data2['status'] != 'Tidak Valid') { ?>
        <a class="btn btn-sm btn-success" data-placement="bottom" data-toggle="tooltip" title="Detail Pembayaran" href="detail-konfirmasi.php?hal=detail&kd=<?php echo $data2['id_kon'];?>">
            <span class="glyphicon glyphicon-search"></span>
        </a>
    <?php } ?>

    <?php if ($data2['status'] != 'Dibatalkan' && $data2['status'] != 'Proses' && $data2['status'] != 'Bayar' && $data2['status'] != 'Tidak Valid') { ?>
        <a class="btn btn-sm btn-primary" data-placement="bottom" data-toggle="tooltip" title="Konfirmasi Pembayaran" href="edit-konfirmasi.php?hal=edit&kode=<?php echo $data2['id_kon'];?>">
