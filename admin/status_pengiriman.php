...
<?php } ?>
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
           ...
           <input type='submit' value='Cari Data' class="btn btn-sm btn-primary" /> <a href='status_pengiriman.php' class="btn btn-sm btn-success" >Refresh</i></a>
            </div>
              </div>
           <!-- /.row -->
                    <br />
                    ...
                    <div class="panel panel-success">
                        ...
                        <h3 class="panel-title"><i class="fa fa-user"></i> Status Pengiriman </h3> 
                        </div>
                        <div class="panel-body">
                       <!-- <div class="table-responsive"> -->
                                   <?php
$query3 = "SELECT po.*, customer.nama AS nama_pelanggan, customer.alamat, customer.no_telp, produk.nama AS nama_produk, produk.harga, SUM(konfirmasi.jumlah) AS jumlah 
           FROM po 
           INNER JOIN customer ON po.kd_cus = customer.kd_cus 
           INNER JOIN produk ON po.kode = produk.kode 
           LEFT JOIN konfirmasi ON po.nopo = konfirmasi.nopo 
           GROUP BY po.nopo";
    ...
    $query3 = "
        SELECT po.*, 
               ...
               produk.harga, 
               SUM(konfirmasi.jumlah) AS jumlah 
        FROM po 
        INNER JOIN customer ON po.kd_cus = customer.kd_cus 
        INNER JOIN produk ON po.kode = produk.kode 
        LEFT JOIN konfirmasi ON po.nopo = konfirmasi.nopo 
        WHERE po.nopo LIKE '%$qcari%' 
           OR customer.nama LIKE '%$qcari%'
        GROUP BY po.nopo, customer.nama, customer.alamat, customer.no_telp, produk.nama, produk.harga
    ";
}
...
while($data2 = mysqli_fetch_array($hasil2)) { 
  if ($data2['status'] == 'Proses' || $data2['status'] == 'Dikirim' || $data2['status'] == 'Selesai') { ?>
    <tr>
      ...
      <td><center><?php echo $data2['no_telp']; ?></center></td>
      ...
      <?php if ($data2['status'] != 'Selesai') { ?>
        <td><center><div id="thanks"><a class="btn btn-sm btn-primary" data-placement="bottom" data-toggle="tooltip" title="Edit Pengiriman" href="edit_pengiriman.php?hal=edit&kd=<?php echo $data2['nopo'];?>"><span class="glyphicon glyphicon-edit"></span></a>  
                      </center></td></tr></div>
        </center></td>
      <?php } ?>
    </tr>
