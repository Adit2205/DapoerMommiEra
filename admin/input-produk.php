...
if(isset($_POST['input'])){
    ...
    if (!empty($_FILES["nama_file"]["tmp_name"])) {
        ...
        if ($file_size <= 5 * 1024 * 1024) {
            // Allow all types of images
            if (strpos($file_type, 'image/') === 0) {
                $gambar = $namafolder . basename($_FILES['nama_file']['name']);
                if (move_uploaded_file($_FILES['nama_file']['tmp_name'], $gambar)) {
                    $sql = "INSERT INTO produk(kode,nama,jenis,harga,keterangan,stok,gambar) VALUES ('$kode','$nama','$jenis','$harga','$keterangan','$stok','$gambar')";
                ...
                echo "File yang anda kirim bukan gambar";
            }
        } else {
            echo "Ukuran gambar yang anda kirim terlalu besar. Maksimal 5 MB.";
        }
    } else {
